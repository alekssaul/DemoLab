...
if [ DemoLab_Infra=="gcp" ]; then
	...
	gcloud compute copy-files `dirname $0`/scripts core@kubernetes-master:~/ 2> /dev/stdout 1> /dev/null 
	gcloud compute ssh core@kubernetes-master --command "~/scripts/prepare-jenkins-data.sh"
	gcloud compute instances detach-disk kubernetes-master --disk jenkins-home 2> /dev/stdout 1> /dev/null
fi
